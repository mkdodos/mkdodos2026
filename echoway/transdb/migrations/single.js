...
async function migrateData() {
  try {
    ...
    // 從 Access 讀取所有資料
    const rows = await access.query("SELECT * FROM stocks");
    console.log(`讀取成功，共 ${rows.length} 筆資料。`);

    // return;
    // --- 關鍵新增：先刪除 PG 中的所有記錄 ---
    console.log("正在清空 PostgreSQL 中的舊資料...");
    // RESTART IDENTITY 會讓 ID 從 1 開始重新計算
    await pgPool.query("TRUNCATE TABLE stocks RESTART IDENTITY CASCADE");
    ...
    for (const row of rows) {
      const insertSql = `
        INSERT INTO stocks (circle1, note_text) 
        VALUES ($1, $2)
        ; 
      `;
      // 對應 Access 中的欄位名稱
